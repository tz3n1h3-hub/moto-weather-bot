...
def get_trend():
    try:
        ...
        url = (
            ...
            f"?lat=53.9045&lon=27.5615&appid={OPENWEATHER_API_KEY}"
            f"&units=metric&lang=ru&cnt=2&_={cache_buster}"
        )
        ...
        owm_url = (
            ...
            f"?lat=53.9045&lon=27.5615&appid={OPENWEATHER_API_KEY}&units=metric&lang=ru"
        )
...
def get_weather():
    try:
        ...
        url = (
            f"https://api.openweathermap.org/data/2.5/weather"
            f"?lat=53.9045&lon=27.5615&appid={OPENWEATHER_API_KEY}"
            f"&units=metric&lang=ru&_={cache_buster}"
        )
...
def analyze_risks(weather, is_forecast=False):
    ...
    if not is_forecast and dew_point is not None:
        diff = temp - dew_point
        if diff <= 0:
            risks.append("🌫️ Точка росы = температуре! Туман, роса на дороге")
            score += 3
...
def send_weather(chat_id):
    ...
    if feels_like != temp_value:
        temp_line = (
            f"🌡️ <b>Температура:</b> {temp_value}°C "
            f"(ощущается как {feels_like}°C, {temp_desc})"
        )
    else:
    ...
    visibility = weather.get("visibility", 10000)
    vis_info = (
        f"\n🌫️ <b>Видимость:</b> {format_visibility(visibility)} "
        f"({get_visibility_rating(visibility)})"
    )
    ...
    if wind_gust and wind_gust > wind_speed:
        if wind_gust > 10:
            wind_line += (
                f"\n⚠️ <b>ОПАСНЫЕ ПОРЫВЫ:</b> до {wind_gust} м/с! "
                f"Держите руль крепче."
            )
        else:
    ...
    gear_list = get_detailed_gear(
        feels_like,
        wind_speed,
        weather.get("is_night", False),
        weather.get("is_rain", False),
...
def send_forecast(chat_id):
    ...
    if forecast.get("wind_gust", 0) > wind_speed:
        wind_line = (
            ...
            f"(порывы до {forecast.get('wind_gust', 0)} м/с, {wind_desc}) — {wind_feeling}"
        )
    else:
        wind_line = f"💨 <b>Ветер:</b> {wind_speed} м/с ({wind_desc}) — {wind_feeling}"
...
"""
    ...
    gear_list = get_detailed_gear(
        avg_temp, wind_speed, False, forecast.get("is_rain", False), None
    )
    gear_text = "\n".join(f"• {g}" for g in gear_list)
    msg += f"\n<b>🛡️ Экипировка:</b>\n{gear_text}"
...
def send_weekly(chat_id):
    ...
    for day in weekly:
        ...
        msg += (
            f"{emoji} <b>{day['weekday']}</b> {day['date']}: "
            f"{day['condition']} {tempsend_str} | 💨 {wind_str}{rain_str}\n"
        )
...
weather        wind, rain = day["wind_s()peed"], day["rain_total"]
        temp`
_max, temp_min = day["-temp_max"], day["temp_min"]
    ...
    if best_day and worst_day:
        ...
        msg += (
            f"✅ <b>Лучший день:</b> {best_day['weekday']} "
            f"({best_day['temp_min']}°...{best_day['temp_max']}°)\n"
        )
        if best_reason:
        ...
        if any(day["temp_min"] < 0 for day in weekly):
            msg += "• 🧥 Тёплая экипировка обязательна\n"
        if any(day["temp_max"] > 30 for day in weekly):
            msg += "• 💧 Пейте больше воды в жаркие дни\n"
